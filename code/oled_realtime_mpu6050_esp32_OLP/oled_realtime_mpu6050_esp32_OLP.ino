...
// WiFi details
//const char *ssid     = "Ai Van";
//const char *password = "tronghienaivan";
const char *ssid     = "ICSE_Child";
const char *password = "icse_child@123";

unsigned long prev;  //some global variables available anywhere in the program
...
#define mqtt_topic_pub "olp/mpu6050"
#define mqtt_topic_sub "olp"
...
//    long led2 = root["led2"];
//    long led3 = root["led3"];
//    String led1 = root["value"];/
    char led1[20];
    strcpy(led1, root["value"]);
    Serial.println(led1);
...
//    }
    ...
    else if (strcmp(led1, "0xa60")==0){
      prev = millis();
      current = millis();
      irsend.sendSony(0xa60, 12);
      while(current - prev < 2000){
        GOFoled.setCursor(10, 56);
        GOFoled.print("led1");
         GOFoled.display();
...
void toServer() {
  ...
  if (fifoCount >= 1024) {
    ...
    // Send to MQTT broker
    client.publish("olp/detectaction", buffer_mqtt);
  }
}
void MPU() {
  if (!dmpReady) return;
...
//touch detect
 Serial.println(touchRead(T0));
  if(touchRead(T0)<30){
    ...
    while(current - prev < 2000){
      GOFoled.setCursor(10, 56);
      GOFoled.print("Thuc hien hanh dong");
       GOFoled.display();
       delay(10);
        GOFoled.clearDisplay();
        current = millis();
...
//  GOFoled.drawBitmap(36, 2, heart2, 56, 56, WHITE);  //draw heart2 bitmap.
  ...
  delay(10);
  GOFoled.clearDisplay();
  //  json_buffer["time"] = month + String("/") + day + String("/") + _year + String(" ") + _hour + String(":") + minute + String(":") + second;
  ...
  // MPU 6050
  //  char ch;
  //  Serial.println("Write a character to start MPU function");
  //  while (Serial.available() && Serial.read()); // empty buffer
  //  while (!Serial.available());                 // wait for data
  //  if (Serial.available()){
  //    ch = Serial.read();
